...
class CakeMigration extends CakeObject
{
		...
		if (empty($options['precheck'])) {
			...
			if (!class_exists($class)) {
				throw new MigrationException($this, sprintf(
					__d('migrations', 'Migration precheck class (%s) could not be loaded.'),
					$options['precheck']
				), E_USER_NOTICE);
			}
			...
			if (!is_a($this->Precheck, 'PrecheckBase')) {
				throw new MigrationException($this, sprintf(
					__d('migrations', 'Migration precheck class (%s) is not a valid precheck class.'),
					$class
				), E_USER_NOTICE);
			}
		}
	...
	public function run($direction)
	{
		if (!in_array($direction, array('up', 'down'))) {
			throw new MigrationException($this, sprintf(
				__d('migrations', 'Migration direction (%s) is not one of valid directions.'),
				$direction
			), E_USER_NOTICE);
		}
	...
	protected function _invokeCallbacks($callback, $type, $data = array())
	{
		...
		if ($callback === 'beforeMigration' || $callback === 'afterMigration') {
			...
			throw new MigrationException($this, sprintf(
				__d('migrations', 'Interrupted when running "%s" callback.'),
				$callback
			), E_USER_NOTICE);
		}
	}
